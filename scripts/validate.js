...
function enableValidation(config) {
  ...
  forms.forEach((form) => {
    ...
    inputEventListener(inputList, submitButton, config.errorTemplateSelector, config.inactiveButtonClass, config.inputErrorClass, config.errorClass);
  });
}


function inputEventListener(inputList, submitButton, errorTemplateSelector, inactiveButtonClass, inputErrorClass, errorClass) {
...
function resetFormErrors (form, { inputSelector, errorTemplateSelector, inputErrorClass, errorClass }) {
  const formInputs = Array.from(form.querySelectorAll(validationConfig.inputSelector))
  formInputs.forEach((input) => {
    const errorContainer = form.querySelector(`${validationConfig.errorTemplateSelector}${input.name}`) // Сформированный селектор
